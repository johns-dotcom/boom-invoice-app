...
BACKUP_TOKEN        = os.environ.get("BACKUP_TOKEN", "")  # secret token for /backup/auto cron endpoint
...
def update_entry(eid):
    allowed = {"in_quickbooks","uploaded_to_stem","artist","song","notes",
               ...
               "payee","amount","description","invoice_date","scheduled_payment_date","paid_by"}
...
def entries():
    try:
        ...
        cur.execute("""SELECT id,invoice_date,payee,description,category,artist,song,
                              ...
                              payment_terms,parent_id,boom_rep,scheduled_payment_date,paid_by
                       FROM expenses
                       WHERE (status = 'approved' OR status IS NULL) AND deleted IS NOT TRUE
        ...
        return jsonify([{"id":r[0],"invoice_date":str(r[1] or ""),"payee":str(r[2] or ""),
                         ...
                         "scheduled_payment_date":str(r[31] or ""),
                         "paid_by":str(r[32] or "")} for r in rows])
    except Exception as e:
        return jsonify({"error":str(e)}), 500
...
def _build_backup_excel(conn, kind):
    """Two-sheet workbook: full expenses dump (all rows/cols) + audit_log."""
    def thin_bdr():
        s = Side(style="thin", color="FFE2E2E2")
    ...
    cur.execute("""
        SELECT id, invoice_date, payee, description, category, artist, song,
               ...
               payment_status, in_quickbooks, qb_entry_date,
               uploaded_to_stem, stem_upload_date,
               status, approved_by, approved_at,
               cobrand, is_reimbursement,
               vendor_submitted, vendor_name, vendor_email, vendor_address,
               w9_filename, invoice_filename, proof_filename,
               notes, payment_terms, artist_breakdown,
               boom_rep, scheduled_payment_date, paid_by,
               parent_id, deleted,
    ...
    exp_headers = [
        "ID", "Invoice Date", "Payee", "Description", "Category", "Artist", "Song",
        "Invoice #", "Amount", "Currency", "Payment Method", "Payment Date",
        "Payment Status", "In QuickBooks?", "QB Entry Date",
        "Uploaded to Stem?", "Stem Upload Date",
        "Status", "Approved By", "Approved At",
        "Cobrand", "Is Reimbursement",
        "Vendor Submitted", "Vendor Name", "Vendor Email", "Vendor Address",
        "W9 Filename", "Invoice Filename", "Proof Filename",
        "Notes", "Payment Terms", "Artist Breakdown",
        "Boom Rep", "Scheduled Payment Date", "Paid By",
        "Parent ID", "Deleted",
    ...
    for col_idx, header in enumerate(exp_headers, 1):
        ...
        cell.fill = PatternFill("solid", start_color="FF333333", end_color="FF333333")
        cell.alignment = Alignment(horizontal="center", vertical="center")
        cell.border = thin_bdr()
    ws1.row_dimensions[1].height = 20

    for row_idx, row in enumerate(exp_rows, 2):
        bg = "FFF9F9F9" if row_idx % 2 == 0 else "FFFFFFFF"
        rf = PatternFill("solid", start_color=bg, end_color=bg)
        for col_idx, val in enumerate(row, 1):
            # Convert date/datetime objects to plain strings for clean output
    ...
    for col in ws1.columns:
        ...
        ws1.column_dimensions[col[0].column_letter].width = min(max_len + 2, 42)
    ...
    cur.execute("""
        SELECT id, timestamp, user_name, action, entry_id, entry_payee,
               field, old_value, new_value, details
        FROM audit_log
        ORDER BY id ASC
    """)
    ...
    audit_headers = [
        "ID", "Timestamp", "User", "Action", "Entry ID", "Entry Payee",
    ...
    for row_idx, row in enumerate(audit_rows, 2):
        bg = "FFF9F9F9" if row_idx % 2 == 0 else "FFFFFFFF"
        rf = PatternFill("solid", start_color=bg, end_color=bg)
        for col_idx, val in enumerate(row, 1):
            if hasattr(val, "strftime"):
    ...
    for col in ws2.columns:
        max_len = max((len(str(c.value or "")) for c in col), default=8)
        ws2.column_dimensions[col[0].column_letter].width = min(max_len + 2, 55)

    return wb


def _send_backup_email(xlsx_bytes, filename):
    """Email a backup Excel to NOTIFY_EMAIL using Gmail OAuth."""
    client_id     = os.environ.get("GMAIL_CLIENT_ID", "")
    client_secret = os.environ.get("GMAIL_CLIENT_SECRET", "")
    refresh_token = os.environ.get("GMAIL_REFRESH_TOKEN", "")
    sender        = os.environ.get("GMAIL_USER", "")
    if not all([client_id, client_secret, refresh_token, sender]):
        raise RuntimeError("Gmail OAuth not configured — set GMAIL_CLIENT_ID, GMAIL_CLIENT_SECRET, GMAIL_REFRESH_TOKEN, GMAIL_USER")
  ...
  <div style='font-size:13px;font-weight:800;color:#111;letter-spacing:-.2px'>boom.</div>
  <div style='height:2px;background:#e31e24;margin:8px 0 16px'></div>
  <h2 style='margin:0 0 10px;font-size:16px;color:#111'>Database Backup</h2>
  <p style='font-size:13px;color:#555;margin:0 0 8px'>Your Boom Records database backup is attached. The spreadsheet contains two sheets: all expense records (including deleted) and the full audit log.</p>
  <p style='font-size:12px;color:#9ca3af;margin:0'>File: {filename}</p>
</div>"""

    msg = MIMEMultipart("mixed")
    msg["Subject"] = f"Boom Records DB Backup — {filename}"
    msg["From"]    = f"Boom.Records <{sender}>"
    msg["To"]      = ", ".join(recipients)
    msg.attach(MIMEText(html, "html"))

    att = MIMEBase("application", "vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    att.set_payload(xlsx_bytes)
    email_encoders.encode_base64(att)
    att.add_header("Content-Disposition", "attachment", filename=filename)
    msg.attach(att)

    import base64 as b64lib2
    raw = b64lib2.urlsafe_b64encode(msg.as_bytes()).decode()
    urllib.request.urlopen(urllib.request.Request(
        "https://gmail.googleapis.com/gmail/v1/users/me/messages/send",
        data=json.dumps({"raw": raw}).encode(),
        headers={"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"},
    ...
    print(f"Backup email sent to {recipients}: {filename}")


@app.route("/admin/backup")
@admin_required
def admin_backup_download():
    """Download a full database backup as a two-sheet Excel file."""
    try:
        conn, kind = get_db()
        wb = _build_backup_excel(conn, kind)
        conn.close()
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    buf = io.BytesIO()
    wb.save(buf)
    buf.seek(0)
    fname = f"BoomRecords_Backup_{date.today():%Y-%m-%d}.xlsx"
...
def admin_backup_email():
    """Email the full database backup to NOTIFY_EMAIL addresses."""
    try:
        conn, kind = get_db()
        wb = _build_backup_excel(conn, kind)
        conn.close()
        ...
        buf = io.BytesIO()
        wb.save(buf)
        _send_backup_email(buf.getvalue(), fname)
        return jsonify({"ok": True, "message": f"Backup emailed: {fname}"})
    except Exception as e:
        app.logger.error("Backup email error: %s", e, exc_info=True)
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/backup/auto")
def backup_auto():
    """Token-protected endpoint for scheduled/cron backup.
    Set BACKUP_TOKEN env var. Hit: GET /backup/auto?token=<value>
    Safe to ping from cron-job.org, Railway cron, GitHub Actions, etc.
    """
    ...
    try:
        ...
        app.logger.error("Auto-backup error: %s", e, exc_info=True)
        return jsonify({"ok": False, "error": str(e)}), 500
...
def submit_invoice():
    ...
    inv_ext = os.path.splitext(inv_fname)[1].lower()
    if inv_ext not in ALLOWED_FILE_EXTENSIONS:
        return err("Invoice must be a PDF, JPG, or PNG file.")
    if len(file_bytes) > MAX_UPLOAD_MB * 1024 * 1024:
        return err(f"Invoice file must be under {MAX_UPLOAD_MB}MB.")
    ...
    if has_new_w9:
        ...
        w9_ext = os.path.splitext(w9_fname)[1].lower()
        if w9_ext not in ALLOWED_FILE_EXTENSIONS:
            return err("W9/W8 must be a PDF, JPG, or PNG file.")
        w9_bytes = w9_file.read()
        if len(w9_bytes) > MAX_UPLOAD_MB * 1024 * 1024:
            return err(f"W9/W8 file must be under {MAX_UPLOAD_MB}MB.")
        w9_b64 = base64.b64encode(w9_bytes).decode()
    else:
        w9_fname = None
        w9_b64 = None
    ...
    if has_receipt:
        ...
        receipt_ext = os.path.splitext(receipt_fname)[1].lower()
        if receipt_ext not in ALLOWED_FILE_EXTENSIONS:
            return err("Receipt must be a PDF, JPG, or PNG file.")
        receipt_bytes = receipt_file.read()
        if len(receipt_bytes) > MAX_UPLOAD_MB * 1024 * 1024:
            return err(f"Receipt file must be under {MAX_UPLOAD_MB}MB.")
        receipt_b64 = base64.b64encode(receipt_bytes).decode()
    else:
        receipt_fname = None
        receipt_b64 = None

    # Duplicate submission guard — same vendor email + invoice number already exists
    inv_num_for_check = fields.get("invoice_number", vendor_inv_num) or ""
    if inv_num_for_check:
        try:
            conn, kind = get_db(); cur = conn.cursor(); ph = "%s" if kind=="pg" else "?"
            cur.execute(f"""SELECT id FROM expenses
                            WHERE LOWER(vendor_email)=LOWER({ph})
                              AND LOWER(invoice_number)=LOWER({ph})
                              AND deleted IS NOT TRUE
                            LIMIT 1""", (vendor_email, inv_num_for_check))
            if cur.fetchone():
                conn.close()
                return err("We already have a submission from you with that invoice number. Reach out to us directly if you need to make a change.")
            conn.close()
        except: pass
